...
function convertToHls(request, inputPath, outPutDir, lessonId, durationMs) {
  return new Promise((resolve, reject) => {
    ...
    const args = [
      ...
      inputPath,
      "-filter_complex",
      "[0:v]split=4[v1][v2][v3][v4];[v1]scale=640:360[v1o];[v2]scale=854:480[v2o];[v3]scale=1280:720[v3o];[v4]scale=1920:1080[v4o]",
      "-map",
      "[v1o]",
      "-map",
      "0:a?",
    ...
    ffmpeg.on("close", async (code) => {
      if (code === 0) {
        await updateProgress(request, lessonId, "transcoding", 100);
        resolve();
      } else {
